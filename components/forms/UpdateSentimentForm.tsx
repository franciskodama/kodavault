...
import { updateSentiment } from '@/lib/actions';
...
export function UpdateSentimentForm({ sentiment }: { sentiment: SentimentType }) {
  ...
  const processForm: SubmitHandler<SentimentType> = async (data) => {
    if (!uid) {
      return console.log('User not logged in 🤷🏻‍♂️');
    }

    const result = await updateSentiment(data);

    if (result) {
      toast({
        title: 'Sentiment Updated! 🎉',
        description: 'Your sentiment indicator has been updated.',
        variant: 'success',
      });
    } else {
      toast({
        title: 'Uh oh! Something went wrong!',
        description: 'Your sentiment indicator was NOT updated.',
        variant: 'destructive',
      });
    }
  ...
  return (
    <form onSubmit={handleSubmit(processForm)} className='pb-8'>
      <div className='flex flex-col'>
        ...
        <div className={classDiv}>
          <label className={classTitle} htmlFor='pair'>
            Pair
          </label>
          <input
            id='pair'
            className={classInput}
            placeholder='ex: USDT'
            {...register('pair', { required: "Pair can't be empty" })}
          />
          {errors.pair?.message && (
            <p className={classError}>{errors.pair.message}</p>
          )}
        </div>
