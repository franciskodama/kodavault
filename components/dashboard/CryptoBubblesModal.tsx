...
export default function CryptoBubblesModal({
  allCryptos,
  userOwnedSymbols,
  onClose,
}: CryptoBubblesModalProps) {
  const [sizeKey, setSizeKey] = useState<'total_volume' | 'market_cap'>(
    'total_volume'
  );
  const [filterType, setFilterType] = useState<
    'all' | 'portfolio' | 'gainers' | 'losers'
  >('all');
  ...
  const nodesWithSearch = useMemo(() => {
    ...
    return nodes.map((n) => ({
      ...n,
      isDimmed:
        !n.symbol.includes(query) && !n.name.toUpperCase().includes(query),
    }));
  }, [nodes, searchQuery]);
    ...
    <div className='flex flex-col h-full bg-slate-950 text-slate-100'>
      ...
      <div className='flex flex-col md:flex-row items-center justify-between gap-4 p-6 border-b border-slate-800 bg-slate-900/60 backdrop-blur'>
        ...
        <div className='flex flex-wrap items-center gap-3 w-full md:w-auto'>
          ...
          <div className='flex bg-slate-950 p-1 rounded-xl border border-slate-800'>
            {(['all', 'portfolio', 'gainers', 'losers'] as const).map(
              (type) => (
                <Button
                  key={type}
                  variant='ghost'
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    transition:
                      'transform 0.2s cubic-bezier(0.16, 1, 0.3, 1), opacity 0.3s ease',
                  }}
                  ...
                  className={`rounded-full flex flex-col items-center justify-center text-white select-none cursor-pointer
                    ...
                    ${
                      node.isDimmed
                        ? 'opacity-25 scale-90'
                        : 'opacity-100 scale-100 hover:scale-110 hover:z-25'
                    }
                  `}
                >
                  {/* Icon image */}
        ...
        {hoveredNode && hoveredCoinDetails ? (
            ...
            <div className='flex items-center gap-4'>
              ...
              <div className='flex flex-col'>
                ...
                <span className='text-xs text-slate-400 font-medium'>
                  {sizeKey === 'total_volume'
                    ? `24h Volume: $${numberFormatterNoDecimals.format(
                        hoveredNode.value
                      )}`
                    : `Market Cap: $${numberFormatterNoDecimals.format(
                        hoveredNode.value
                      )}`}
                </span>
              </div>
            ...
            <div className='flex items-center gap-8'>
              {hoveredNode.isPortfolio && (
                <div className='flex items-center gap-2 px-3 py-1 bg-yellow-500/10 border border-[#DDF906]/35 rounded-xl'>
                  ...
                  <span className='text-xs font-semibold text-[#DDF906]'>
                    Portfolio Asset
                  </span>
                </div>
              )}
              ...
              <div className='flex flex-col items-end'>
                ...
                <div
                  className={`flex items-center gap-1 font-extrabold text-base
                  ${
                    hoveredNode.priceChange >= 0
                      ? 'text-emerald-400'
                      : 'text-rose-400'
                  }
                `}
                >
                  ...
                  <span>
                    {hoveredNode.priceChange >= 0 ? '+' : ''}
                    {hoveredNode.priceChange.toFixed(2)}%
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className='flex items-center justify-between w-full text-slate-400 text-xs font-medium'>
            <p className='flex items-center gap-2'>
              ...
              Hover over a bubble to inspect real-time performance and portfolio
              statistics.
            </p>
