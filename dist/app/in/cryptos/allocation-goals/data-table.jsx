...
function DataTable({ columns, data, sumGoals, }) {
    var _a;
    ...
    return (<div className='rounded-sm border border-slate-200  bg-white'>
      <table_1.Table>
        <table_1.TableHeader>
          {table.getHeaderGroups().map((headerGroup) => (<table_1.TableRow key={headerGroup.id}>
              {headerGroup.headers.map((header) => {
                return (<table_1.TableHead key={header.id} className='text-center'>
                    {header.id === 'actionGoal' ? (<div className='flex items-center gap-2'>
                        {header.isPlaceholder
                            ? null
                            : (0, react_table_1.flexRender)(header.column.columnDef.header, header.getContext())}
                        <div className={`${sumGoals > 100 &&
                            'bg-red-500 text-white py-1 px-1 rounded-[2px]'} text-center text-xs font-base text-primary`}>
                          ({sumGoals} %)
                        </div>
                      </div>) : (<>
                        {header.isPlaceholder
        ...
        <table_1.TableBody>
          {((_a = table.getRowModel().rows) === null || _a === void 0 ? void 0 : _a.length) ? (table.getRowModel().rows.map((row) => (<table_1.TableRow key={row.id} data-state={row.getIsSelected() && 'selected'}>
