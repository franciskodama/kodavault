  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    headers.forEach((header, index) => {
      ...
      if (val === 'null' || val === '' || val === 'N/A') {
        ...
        obj[header] = false;
      } else if (!isNaN(Number(val)) && val.length > 0 && !val.includes('-') && !val.includes(':')) {
...
async function main() {
  ...
  for (const asset of assets) {
    ...
    } catch (e) { console.error(`Failed asset ${asset.asset}:`, e); }
  }
  ...
  for (const s of shortcuts) {
    try {
        const { color, ...data } = s; // Skip 'color' as it's no longer in schema
        if (data.created_at) data.created_at = new Date(data.created_at);
  ...
  for (const ka of keyAssets) {
    ...
    } catch (e) { console.error(`Failed keyAsset ${ka.asset}:`, e); }
  }

  console.log('--- Migrating Projections ---');
  const projections = parseCSV(path.join(dataDir, 'Projection.csv'));
  for (const p of projections) {
    try {
        const data: any = { ...p };
        if (data.created_at) data.created_at = new Date(data.created_at);
        await prisma.projection.upsert({
            where: { uid_asset: { uid: p.uid, asset: p.asset } },
            update: data,
            create: data
        });
    } catch (e) { console.error(`Failed projection ${p.asset}:`, e); }
  }

  console.log('--- Migrating CoinGoals ---');
  const coinGoals = parseCSV(path.join(dataDir, 'CoinGoal.csv'));
  for (const cg of coinGoals) {
    try {
        const data: any = { ...cg };
        if (data.created_at) data.created_at = new Date(data.created_at);
        await prisma.coinGoal.upsert({
            where: { id: cg.id },
            update: data,
            create: data
        });
    } catch (e) { console.error(`Failed coinGoal ${cg.coin}:`, e); }
  }
