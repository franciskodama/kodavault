...
// Manual env loading with robust parsing
const envFile = fs.readFileSync('.env', 'utf-8');
const searchString = 'DATABASE_URL=';
const line = envFile.split('\n').find(l => l.startsWith(searchString));
const connectionString = line ? line.substring(searchString.length).replace(/"/g, '').trim() : null;
...
function parseCSV(filePath: string) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    headers.forEach((header, index) => {
      ...
      else if (!isNaN(Number(val)) && val.length > 0 && !val.includes('-') && !val.includes(':') && !val.includes('/') && !val.includes(' ')) obj[header] = Number(val);
      else obj[header] = val;
    });
    results.push(obj);
  }
...
async function main() {
  ...
  for (const g of goals) {
    try {
      if (g.created_at) g.created_at = new Date(g.created_at);
      if (g.goal) g.goal = Number(g.goal);
      await prisma.goal.upsert({ where: { uid: g.uid }, update: g, create: g });
    } catch (e: any) { console.error(`Failed goal ${g.uid}: ${e.message}`); }
  }
